...
> 收录的字节跳动面经同学 1 Java 后端技术一面面试原题：你平时用到的数据库
> 收录的腾讯云智面经同学 16 一面面试原题：数据库用过哪些，对哪个比较熟？
> 收录的 360 面经同学 3 Java 后端技术一面面试原题：用过哪些数据库
> 收录的招商银行面经同学 6 招银网络科技面试原题：了解 MySQL、Redis 吗？
> 收录的国企零碎面经同学 9 面试原题：数据库用什么多（说了 Mysql 和 Redis）
...
> 收录的腾讯 Java 后端实习一面原题：请说说 MySQL 的内联、左联、右联的区别。

### 3.说一下数据库的三大范式？

三大范式的作用是为了减少数据冗余，提高数据完整性。
![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-16e74a6b-a42a-464e-9b10-0252ee7ecc6e.jpg)

①、第一范式（1NF）：确保表的每一列都是不可分割的基本数据单元，比如说用户地址，应该拆分成省、市、区、详细信息等 4 个字段。

![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240418093235.png)

②、第二范式（2NF）：在 1NF 的基础上，要求数据库表中的每一列都和主键直接相关，而不能只与主键的某一部分相关（主要针对联合主键）。

比如说在一个订单表中，可能会存在订单编号和商品编号，设计出来的表可能是这样的。
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240418093351.png)

这个订单表中就存在冗余数据，比如说商品名称、单位、商品价格等，应该将其拆分为订单表、订单商品关联表、商品表。

![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240418093726.png)
...
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240418094332.png)

在实际开发过程中，三大范式有时候反而成为了老太婆的裹脚布，让表的设计变得复杂而又啰嗦，就像第二范式中提到的订单详情表，把商品价格和名称放在订单表中可以减少额外的 JOIN 操作，提高查询效率。

> 收录的字节跳动面经同学 13 Java 后端二面面试原题：什么是三大范式，为什么要有三大范式，什么场景下不用遵循三大范式，举一个场景

### 4.varchar 与 char 的区别？

![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-40f42d59-a295-4543-8a03-43925da4d6d9.jpg)
...
1. not in 和 not exists：如果查询语句使用了 not in，那么内外表都进行全表扫描，没有用到索引；而 not extsts 的子查询依然能用到表上的索引。所以无论那个表大，用 not exists 都比 not in 要快。
...
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240418103116.png)

   > 1. 收录的字节跳动面经同学 13 Java 后端二面面试原题：mysql 怎么存 emoji，怎么编码
...
![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-2c754ee2-20c4-4c03-9db0-22c7c9eb7f01.jpg)
...
1. **ON** ：对虚拟表 VT1 应用 ON 筛选，只有那些符合<join_condition>的行才被插入虚拟表 VT2 中
...
1. **WHERE** ：对虚拟表 VT3 应用 WHERE 过滤条件，只有符合<where_condition>的记录才被插入虚拟表 VT4 中
1. **GROUP BY** ：根据 GROUP BY 子句中的列，对 VT4 中的记录进行分组操作，产生 VT5
1. **CUBE|ROLLUP** ：对表 VT5 进行 CUBE 或 ROLLUP 操作，产生表 VT6
1. **HAVING** ：对虚拟表 VT6 应用 HAVING 过滤器，只有符合<having_condition>的记录才被插入虚拟表 VT7 中。
1. **SELECT** ：第二次执行 SELECT 操作，选择指定的列，插入到虚拟表 VT8 中
1. **DISTINCT** ：去除重复数据，产生虚拟表 VT9
1. **ORDER BY** ：将虚拟表 VT9 中的记录按照<order_by_list>进行排序操作，产生虚拟表 VT10。11）
1. **LIMIT** ：取出指定行的记录，产生虚拟表 VT11，并返回给查询用户

### 67.介绍一下 MySQL 的常用命令（补充）

   > 1. 2024 年 03 月 13 日增补，可以先向面试官确认一下，“您提到的常用命令是指数据库表的增删改查 SQL 吗？”得到确认答复后可以根据下面这张思维导图作答：
...
`ROLLBACK;`
   ...
   > 1. 收录的用友金融一面原题：介绍一下 MySQL 的常用命令

### 68.介绍一下 MySQL bin 目录下的可执行文件（补充）

   > 1. 2024 年 03 月 13 日增补
...
> 收录的美团面经同学 16 暑期实习一面面试原题：MySQL 第 3-10 条记录怎么查？
...
> 收录的华为 OD 面经同学 1 一面面试原题：用过哪些 MySQL 函数？
> 收录的 小公司面经合集好未来测开面经同学 3 测开一面面试原题：知道 MySQL 的哪些函数，如 order by count()
...
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240425111246.png)

可以通过显式转换来规避这种情况。

`SELECT CAST('1' AS SIGNED INTEGER) + 1; -- 结果为 2`

> 收录的小公司面经合集同学 1 Java 后端面试原题：说说 SQL 的隐式数据类型转换？
...
> 收录的美团面经同学 2 Java 后端技术一面面试原题：MySQL 执行语句的整个过程了解吗？
> 收录的美团面经同学 18 成都到家面试原题：mysql一条数据的查询过程
...
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240515123301.png)
...
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240408074630.png)

> 收录的字节跳动面经同学 1 Java 后端技术一面面试原题：MySQL 支持哪些存储引擎?
> 收录的用友面试原题：innodb 引擎和 hash 引擎有什么区别
> 收录的国企零碎面经同学 9 面试原题：MySQL 的存储引擎
...
![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-b7aa040e-a3a7-4133-8c43-baccc3c8d012.jpg)
...
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240403130104.png)

InnoDB 为聚簇索引，索引和数据不分开。

![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240403130508.png)
...
> 收录的字节跳动面经同学 1 Java 后端技术一面面试原题：MyISAM 和 InnoDB 的区别有哪些？
...
![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-c0ef6e68-bb33-48fc-b3a2-b9cdadd8e403.jpg)
...
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240417074049.png)
...
> 收录的华为面经同学 8 技术二面面试原题：MySQL 中的 bin log 的作用是什么？
> 收录的美团面经同学 2 Java 后端技术一面面试原题：说说 MySQL 的三大日志？
...
> 收录的美团同学 2 优选物流调度技术 2 面面试原题：redo log、bin log
...
![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-75d5226b-cab9-461a-89a9-befcb2dfb996.jpg)
...
![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-869c309b-9b93-46e1-8414-b35128e287a5.jpg)
...
![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-e1f59341-0695-45db-b759-30db73314e39.jpg)
...
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240327083506.png)
...
1. 如果查询缓存打开则会优先查询缓存，如果缓存中有对应的结果，直接返回给客户端。不过，MySQL 8.0 版本已经移除了查询缓存。
1. 分析器对 SQL 语句进行语法分析，判断是否有语法错误。
1. 搞清楚 SQL 语句要干嘛后，MySQL 还会通过优化器生成执行计划。
1. 执行器调用存储引擎的接口，执行 SQL 语句。
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240327083838.png)
...
![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-c0c43f82-3930-44f0-9abc-b33b08c02d2d.jpg)
...
> 收录的腾讯云智面经同学 16 一面面试原题：场景题：sql 查询很慢怎么排查
> 收录的快手面经同学 5 面试原题：慢sql日志怎么开启？

### 25.有哪些方式优化 SQL？

我在进行 SQL 优化的时候，主要通过以下几个方面进行优化：
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240327104050.png)
...
> 收录的腾讯面经同学 22 暑期实习一面面试原题：查询优化、联合索引、覆盖索引
> 收录的华为面经同学 8 技术二面面试原题：说说 SQL 该如何优化
> 收录的华为面经同学 6 Java 通用软件开发一面面试原题：说说 SQL 该如何优化
> 收录的微众银行同学 1 Java 后端一面的原题：MySQL 索引如何优化？
> 收录的携程面经同学 10 Java 暑期实习一面面试原题：讲一讲 MySQL 的索引，如何优化 SQL？
> 收录的用友面试原题：了解 mysql 怎么优化吗
...
示例：
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240417092646.png)

> 收录的华为面经同学 8 技术二面面试原题：怎么看走没走索引，如何分析 SQL
...
![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-6b9c9901-9bf3-46ed-a5c4-c1b781965c1e.jpg)

可通过 `create index` 创建索引，比如：

`create index idx_name on students(name);`

> 收录的腾讯面经同学 23 QQ 后台技术一面面试原题：MySQL 索引，为什么用 B+树
> 收录的小米面经同学 E 第二个部门 Java 后端技术一面面试原题：为什么需要索引
> 收录的小公司面经同学 5 Java 后端面试原题：数据库索引讲一下，然后为什么会加快查询速度，我讲到了 B+树，然后问了 B 数与 B+树区别
...
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240312094537.png)
...
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240311231652.png)

②、非聚簇索引：它的叶子节点只包含一个主键值，通过非聚簇索引查找记录要先找到主键，然后通过主键再到聚簇索引中找到对应的记录行，这个过程被称为回表。

![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240311231611.png)
...
> 收录的科大讯飞非凡计划研发类面经原题：聊聊 MySQL 的索引
> 收录的腾讯面经同学 23 QQ 后台技术一面面试原题：MySQL 索引，为什么用 B+树
> 收录的携程面经同学 10 Java 暑期实习一面面试原题：讲一讲 MySQL 的索引，如何优化 SQL？
> 收录的阿里面经同学 5 阿里妈妈 Java 后端技术一面面试原题：索引的分类，创建索引的最佳实践
> 收录的 360 面经同学 3 Java 后端技术一面面试原题：mysql 的索引用过哪些
> 收录的用友面试原题：索引是什么？有哪些索引
...
> 收录的用友金融一面原题：索引的作用，加索引需要注意什么
> 收录的京东同学 10 后端实习一面的原题：查询和更新都频繁的字段是否适合创建索引，为什么
> 收录的小米春招同学 K 一面面试原题：索引怎么设计才是最好的
> 收录的京东面经同学 1 Java 技术一面面试原题：MySQL 索引结构，建立索引的策略
> 收录的阿里面经同学 5 阿里妈妈 Java 后端技术一面面试原题：索引的分类，创建索引的最佳实践
...
> 收录的字节跳动面经同学 1 Java 后端技术一面面试原题：where b =5 是否一定会命中索引？（索引失效场景）
> 收录的京东面经同学 1 Java 技术一面面试原题：索引失效的情况
> 收录的小公司面经合集同学 1 Java 后端面试原题：编写 SQL 语句哪些情况会导致索引失效？
...
> 收录的字节跳动面经同学 1 技术二面面试原题：性别字段要建立索引吗？为什么？什么是区分度？MySQL查看字段区分度的命令？
...
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240322140825.png)

B 树的节点大小通常与页的大小对齐，这样每次从磁盘加载一个节点时，可以正好是一个页的大小。因为 B 树的节点可以有多个子节点，可以填充更多的信息以达到一页的大小。

![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240322141957.png)
...
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240322142950.png)
...
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240516125249.png)

B+树的特点是非叶子节点只存储索引，叶子节点存储数据，并且相邻的叶子节点之间有指针链接。

![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240516125326.png)
...
> 收录的字节跳动商业化一面的原题：说说 B+树，为什么 3 层容纳 2000W 条，为什么 2000w 条数据查的快
> 收录的国企面试原题：说说 MySQL 的底层数据结构，B 树和 B+树的区别
> 收录的腾讯面经同学 22 暑期实习一面面试原题：MySQL 为什么选用 B+树
> 收录的小米面经同学 E 第二个部门 Java 后端技术一面面试原题：说一说 mysql 索引的底层机制
> 收录的京东面经同学 1 Java 技术一面面试原题：MySQL 索引结构，建立索引的策略
> 收录的腾讯云智面经同学 16 一面面试原题：MySQL 索引结构，为什么用 B+树？
> 收录的小公司面经同学 5 Java 后端面试原题：数据库索引讲一下，然后为什么会加快查询速度，我讲到了 B+树，然后问了 B 数与 B+树区别
> 收录的百度面经同学 1 文心一言 25 实习 Java 后端面试原题：B+树的页是单向链表还是双向链表？如果从大值向小值检索，如何操作？
> 收录的得物面经同学 1 面试原题：项目索引，MySQL索引，mongoDB为什么用的B树，二者比较

### 34.那一棵 B+树能存储多少条数据呢？

推荐阅读：[清幽之地：InnoDB 一棵 B+树可以存放多少行数据？](https://juejin.cn/post/6904293886626103309)
![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-16f3523d-20b0-4376-908d-ac40b329768f.jpg)
...
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240410124358.png)

如果有 2KW 条数据，那么这颗 B+树的高度为 3 层。

> 收录的字节跳动商业化一面的原题：说说 B+树，为什么 3 层容纳 2000W 条，为什么 2000w 条数据查的快
> 收录的奇安信面经同学 1 Java 技术一面面试原题：innodb 使用数据页存储数据？默认数据页大小 16K，我现在有一张表，有 2kw 数据，我这个 b+树的高度有几层？
> 收录的美团面经同学 18 成都到家面试原题：一张表最多存多少数据（我答得2kw，根据b+树的三层高度计算）
> 收录的得物面经同学 1 面试原题：MySQL B+树的度数越大越好吗，一般设多少
...
> 收录的百度面经同学 1 文心一言 25 实习 Java 后端面试原题：MySQL 索引为什么使用 B+树而不是用别的数据结构？
...
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240325115641.png)

这种结构使得 B+树非常适合进行范围查询——一旦到达了范围的开始位置，接下来的元素可以通过遍历叶子节点的链表顺序访问，而不需要回到树的上层。如 SQL 中的 ORDER BY 和 BETWEEN 查询。

![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240325115614.png)
...
> 收录的支付宝面经同学 2 春招技术一面面试原题：聚簇索引和非聚簇索引的区别？B+树叶子节点除了存数据还有什么？
> 收录的奇安信面经同学 1 Java 技术一面面试原题：b 树和 b+树有什么区别
> 收录的百度面经同学 1 文心一言 25 实习 Java 后端面试原题：MySQL 索引为什么使用 B+树而不是用别的数据结构？
...
> 收录的美团面经同学 16 暑期实习一面面试原题：MySQL 模糊查询怎么查，什么情况下模糊查询不走索引

### 38.聚簇索引与非聚簇索引的区别？

在 MySQL 的 InnoDB 存储引擎中，主键就是聚簇索引。聚簇索引不是一种新的索引，而是一种**数据存储方式** 。
![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-692cced2-615a-4b70-a933-69771d53e809.jpg)
...
> 收录的小米春招同学 K 一面面试原题：mysql：聚簇索引和非聚簇索引区别
> 收录的支付宝面经同学 2 春招技术一面面试原题：聚簇索引和非聚簇索引的区别？B+树叶子节点除了存数据还有什么？
> 收录的字节跳动面经同学 1 Java 后端技术一面面试原题：聚簇索引是什么？非聚簇索引是什么？
> 收录的腾讯云智面经同学 16 一面面试原题：聚簇索引和非聚簇索引的区别？
> 收录的快手面经同学 1 部门主站技术部面试原题：Mysql 的聚簇索引和非聚簇索引的区别是什么?
...
> 收录的字节跳动面经同学 1 Java 后端技术一面面试原题：使用非聚簇索引如何查找数据？
> 收录的字节跳动面经同学 1 技术二面面试原题：回表记录越多好吗？（回表的代价）
...
![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-69e33c61-34bc-4f4b-912b-ca7beb9d5c7c.jpg)
...
> 收录的比亚迪面经同学 3 Java 技术一面面试原题：说一下数据库索引，最左匹配原则和索引的结构
> 收录的腾讯云智面经同学 16 一面面试原题：说说最左前缀原则
...
1. 对于 `B IN (?, ?)`：这个条件指定了 `B` 列可以取两个可能的值。MySQL 会利用索引来查找所有匹配 `A=?` 且 `B` 列为这两个值中任意一个的记录。
1. 对于 `C>?`：这个条件是一个范围查询。在已经根据 `A` 和 `B` 筛选的基础上，MySQL 会继续利用索引来查找 `C` 列值大于指定值的记录。
...
`EXPLAIN SELECT * FROM tbn WHERE A=1 AND B IN (2, 3) AND C>3\G`![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240315140807.png)
...
示例 1（a=1,c=1）：
`EXPLAIN SELECT * FROM tbn WHERE A=1 AND C=1\G`![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240319131120.png)
...
示例 2（b=1,c=1）：
`EXPLAIN SELECT * FROM tbn WHERE B=1 AND C=1\G`![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240319131245.png)
...
示例 3（a=1,c=1,b=1）：
`EXPLAIN SELECT * FROM tbn WHERE A=1 AND C=1 AND B=1\G`![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240319131306.png)
...
`EXPLAIN SELECT * FROM tbn WHERE B=1 AND C=1\G`![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240408092425.png)
...
`EXPLAIN SELECT * FROM tbn WHERE C=5\G`![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240408092646.png)

> 收录的字节跳动商业化一面的原题：(A,B,C) 联合索引 `select * from tbn where a=? and b in (?,?) and c>?` 会走索引吗？
> 收录的京东同学 10 后端实习一面的原题：联合索引 abc，a=1,c=1/b=1,c=1/a=1,c=1,b=1 走不走索引
> 收录的快手面经同学 7 Java 后端技术一面面试原题：联合索引的一个场景题：(a,b,c)联合索引，(b,c)是否会走索引
> 收录的字节跳动面经同学 1 Java 后端技术一面面试原题：建立联合索引(a,b,c)，where c = 5 是否会用到索引？为什么？
...
![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-a07e4525-ccc1-4287-aec5-ebf3f277857c.jpg)
...
> 收录的小公司面经合集同学 1 Java 后端面试原题：乐观锁和悲观锁，库存的超卖问题的原因和解决方案？
...
![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-8989ac27-e442-4c14-81ad-6bc133d78bfd.jpg)

**Gap Lock 间隙锁**

间隙锁(Gap Locks) 的间隙指的是两个记录之间逻辑上尚未填入数据的部分,是一个**左开右开空间** 。
![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-d60f3a42-4b0f-4612-b7ad-65191fecb852.jpg)
...
![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-ae8a21cc-8b52-467d-9173-4e01b24e04b9.jpg)
...
![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-751425cb-daba-4da1-bab6-f843254cad3d.jpg)
...
![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-31f7f49c-1e5a-4d42-b8b3-e022b3ba82ae.jpg)
...
![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-eaafb8b8-fbe6-42c0-9cc2-f2e04631b56c.jpg)
...
> 收录的京东同学 10 后端实习一面的原题：事务的四个特性，怎么理解事务一致性
> 收录的美团面经同学 16 暑期实习一面面试原题：MySQL 事务是什么，默认隔离级别，什么是可重复读？
> 收录的腾讯面经同学 23 QQ 后台技术一面面试原题：MySQL 事务，隔离级别
> 收录的字节跳动面经同学 1 Java 后端技术一面面试原题：什么是数据库事务？事务的作用是什么？
...
> 收录的美团同学 2 优选物流调度技术 2 面面试原题：MySQL ACID 哪些机制来保证
...
> 收录的美团面经同学 16 暑期实习一面面试原题：MySQL 事务是什么，默认隔离级别，什么是可重复读？
> 收录的腾讯面经同学 23 QQ 后台技术一面面试原题：MySQL 事务，隔离级别
> 收录的快手面经同学 7 Java 后端技术一面面试原题：说一下事务的四大隔离级别，分别解决什么问题
> 收录的字节跳动面经同学 1 Java 后端技术一面面试原题：MySQL 默认隔离级别？
> 收录的美团面经同学 2 Java 后端技术一面面试原题：说说 MySQL 事务的隔离级别，如何实现？
> 收录的小公司面经合集同学 1 Java 后端面试原题：MySQL 的四个隔离级别以及默认隔离级别？
> 收录的 360 面经同学 3 Java 后端技术一面面试原题：数据库隔离级别有哪些？mysql 是属于哪个隔离级别
> 收录的联想面经同学 7 面试原题：Mysql 四个隔离级别，MVCC 实现
...
> 收录的京东面经同学 7 京东到家面试原题：mysql事务隔离级别，默认隔离级别，如何避免幻读
...
> 收录的美团面经同学 2 Java 后端技术一面面试原题：说说 MVCC，解决了什么问题？
> 收录的小公司面经合集同学 1 Java 后端面试原题：了解的 MVCC 吗？
> 收录的联想面经同学 7 面试原题：Mysql 四个隔离级别，MVCC 实现，如果两个AB事务并发修改一个变量，那么A读到的值是什么，怎么分析，快照读的原理，读已提交和可重复读区别，具体原理是什么。
...
![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-771eb01f-3f1a-4437-8e1b-affe4de36ec3.jpg)
...
其基本架构是：
![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-f2313613-25bd-4065-8f63-969a4b5757a7.jpg)
...
> 收录的腾讯云智面经同学 16 一面面试原题：MySQL 的主从复制过程
...
![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-2a43af18-617b-4502-b66a-894c2ff4c6c3.jpg)

②、水平分库：按照一定的策略将一个表中的数据拆分到多个库中，例如，按照用户 id 的 hash 值将用户表拆分到不同的库中。

![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-debe0fb1-d7f7-4ef2-8c99-13c9377138b6.jpg)

   > 1. 收录的快手面经同学 7 Java 后端技术一面面试原题：分库分表了解吗
收录的华为面经同学 8 技术二面面试原题：说说分库分表的准则
...
![](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/sidebar/sanfene/mysql-fcd34332-d38d-455a-875d-d4afd37cac72.jpg)
...
> 收录的腾讯面经同学 24 面试原题：项目中的水平分表是怎么做的？分片键具体是怎么设置的？
...
1. 分布式 ID，比较出名的就是 Twitter 开源的 sonwflake 雪花算法
...
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240410105325.png)
...
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240410105512.png)
...
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240410105951.png)

> 收录的奇安信面经同学 1 Java 技术一面面试原题：一张表：id，name，age，sex，class，sql 语句：所有年龄为 18 的人的名字？找到每个班年龄大于 18 有多少人？找到每个班年龄排前两名的人？
...
> 收录的京东面经同学 1 Java 技术一面面试原题：有一个查询需求，MySQL 中有两个表，一个表 1000W 数据，另一个表只有几千数据，要做一个关联查询，如何优化
...
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240412083019.png)
...
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240412083312.png)
...
> 收录的农业银行面经同学 7 Java 后端面试原题：数据库是先建立索引还是先插入数据
...
> 收录的字节跳动面经同学 13 Java 后端二面面试原题：什么是深分页，select /* from tbn limit 1000000000 这个有什么问题，如果表大或者表小分别什么问题
...
> 收录的字节跳动面经同学 13 Java 后端二面面试原题：一个表（name, sex,age,id），select age,id,name from tblname where name='paicoding';怎么建索引
...
![](https://cdn.tobebetterjavaer.com/stutymore/mysql-20240423113508.png)

> 收录的腾讯云智面经同学 16 一面面试原题：SQL 题：一个学生成绩表，字段有学生姓名、班级、成绩，求各班前十名
