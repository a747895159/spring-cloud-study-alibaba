...
- MyBatis 无法区分对应的 SQL，启动时会抛出异常（如 BindingException: Invalid bound statement）。
...
- 每当执行这4种接口对象的方法时，就会进入拦截方法，具体就是InvocationHandler的invoke()方法，只会拦截那些你指定需要拦截的方法。
>
...
- 编写插件：实现Mybatis的Interceptor接口并重写intercept()方法，然后在给插件编写注解，指定要拦截哪一个接口的哪些方法即可，记住，别忘了在配置文件中配置你编写的插件。@Intercepts 和 @Signature 注解 其匹配机制极为严格，依赖于 **类型、方法名、参数列表三者完全一致**。
- 这4个方法实例化对应的对象之后，都会调用interceptorChain的pluginAll方法，遍历所有的拦截器，然后调用各个拦截器的plugin方法。
- **静态配置(自定义插件)与JDK动态代理的责任链模式(洋葱模型责任链，而非链表指针完成)**,层层代理(嵌套形式:proxy3(proxy2(proxy1(original))))：
>
> 1.在启动时，通过解析插件配置完成拦截器收集与代理链预组装；
> 2.在运行时，借助 JDK 动态代理与责任链模式，实现无侵入式的逻辑织入；
> 3.整个过程无需修改原有代码，却能达到类似 AOP 的效果。
>
