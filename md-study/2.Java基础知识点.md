...
- 布隆过滤器(Bloom Filter):  通过使用多个哈希函数将元素映射到位数组的不同位置上，并将这些位置的比特位设为1。它适用于可以接受一定误报率的场景。
    ...
    - 使用场景：缓存穿透拦截（判断 key 是否不存在）、黑名单过滤（如恶意 IP）、URL去重、邮件垃圾过滤器
```
...
- 位图(BitMap): **状态判断**，用于快速查找和存储大量数据的数据结构,非常适合于大规模数据的快速查找和去重.
  - 空间效率极高。
  - 时间复杂度稳定 O(1)。
  - 完全无误判。
  - 使用场景：大规模 ID 去重、签到、打卡、活跃用户统计、连续登录天数
...
- Redis 的 CuckooFilter (布谷鸟过滤器)：在某些场景下能够比布隆过滤器提供更好的填充率，并且支持了删除元素，也存在误判的概率。
  - 使用场景：需要删除的布隆替代品、大型KV系统过滤器、精度要求更高的存在性判断


| 特性          | Bloom Filter | Cuckoo Filter | Bitmap     |
| ----------- | ------------ | ------------- | ---------- |
| **误判**      | 有            | 更低            | 无          |
| **漏判**      | 无            | 无             | 无          |
| **删除支持**    | ❌（标准版不支持）    | ✔             | ✔          |
| **空间效率**    | 非常高          | 更高            | 范围固定，可能浪费  |
| **适用数据**    | 任意 Key       | 任意 Key        | 只能整数 ID    |
| **适用于稀疏数据** | ✔            | ✔             | ❌          |
| **是否支持扩容**  | 较麻烦          | 较容易           | 位图可手动扩展    |
| **典型场景**    | 缓存穿透         | 高端 KV 数据库     | 活跃用户、ID 去重 |
...
- 线程池队列不宜过大,可以参考: 队列容量 = 线程池每秒能处理的任务数 * 最大容忍延迟时间。
...
- 拒绝策略的使用。
- CPU密集型N+1，IO密集型 2N， 实际计算公式：N*(线程等待时间+线程计算时间)/线程计算时间。
- IO相关调用(HTTP、RPC、Redis等)设置合理的超时配置，避免线程假死。

### Eager（急切）模式 vs 标准线程池对比
**重写队列的 offer() 方法，主动拒绝入队以触发线程创建** 。示例: **Apache Dubbo** :org.apache.dubbo.common.threadpool.support.eager.EagerThreadPool

- 1、自定义任务队列 TaskQueue: 继承 LinkedBlockingQueue ,重写 offer(Runnable) 方法
- 2、在 offer 中判断是否允许入队: 如果当前线程数 < 最大线程数 → 返回 false（假装队列满）,触发线程池走“创建新线程”路径 ,只有当线程真打满了，才允许入队
- 3、配合自定义线程池统计提交任务数 : 跟踪“已提交但未完成”的任务量，辅助决策.

Eager模式简单实现方式: 将核心线程数和最大线程数设置一样,`allowCoreThreadTimeOut(true)` 方式 实现线程回收。
...
### 线程池优雅停机
通过 shutdown() + awaitTermination() 配合 JVM Shutdown Hook 实现可控停机，并对关键任务前置持久化、支持补偿重试，构建“主防+保险”双层防护体系。
