...
- **在每次拉取消息时、或者返回不合法的偏移量OFFSET_ILLEGAL时，则更新offset并持久化到远程Broker。**
...
- 8.数据写入：
    ...
    - Kafka**每个partition独占一个目录，每个partition均有各自的数据文件.log**；引入了日志分段(每段1GB),每个日志文件对应两个索引文件（偏移量索引文件、时间戳索引文件），提高消息查询效率。
