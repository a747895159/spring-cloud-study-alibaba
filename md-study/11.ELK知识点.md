...
**主节点（Master Nodes）**：node.master设置为True（默认）。通常建议设置3个专用的可竞争的主节点。这些节点负责集群的元数据管理，如索引的创建与删除、分片的分配等。主节点不推荐参与数据的存储和检索，因此它们的硬件资源可以相对较轻。
...
ES算是AP模型，主节点仅负责集群元数据管理，如索引的创建和删除、分片的分配等。当主节点挂了，也不影响数据查询，不分新索引写入操作会受影响。
