...
- 字符串常量池还在堆中
- **元空间-方法区常量池**
  - 字面量:开发人员的定义的字面量，如文本字符串、声明为final的常量值。
  - 符号引用量：类和接口全限定名、字段（方法）名称和描述符
  - 运行时常量池：基本类型包装类、String类型
...
# 6.String str1 = new String(“Hello”); 为例来说明不同数据存放在 jvm 不同区域
