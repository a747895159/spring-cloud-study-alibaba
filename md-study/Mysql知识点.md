...
- 范围查询后的插入： 当一个事务执行了一个范围查询（如 SELECT * FROM table WHERE column BETWEEN value1 AND value2）后，如果另一个并发事务在这个范围中插入了新行并提交，那么第一个事务如果再次执行相同的范围查询，会看到新增的行，尽管这些行在第一次查询时并不存在。这是因为MVCC机制在可重复读级别下主要解决了不可重复读问题，但对于新插入的行，除非使用了锁（如 SELECT ... FOR UPDATE 或 SELECT ... LOCK IN SHARE MODE），否则还是可能观察到幻读现象。
- 当前读： 如果事务A执行了一个查询，没有找到满足条件的行，此时事务B插入了一条新记录并提交，事务A后续使用当前读进行查询的时候，就会发现两次查询的记录条目不一样了，这也就产生了幻读。
