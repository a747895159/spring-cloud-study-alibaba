...
Spring 使用 HandlerMapping 接口作为抽象策略，定义“根据请求信息查找处理器”的标准行为。 不同的实现类则代表不同的匹配策略.
  - RequestMappingHandlerMapping 负责解析基于注解的映射（如 @GetMapping("/user")），
  - SimpleUrlHandlerMapping 支持 XML 配置下的 URL 与 Controller Bean 的静态绑定。

HandlerAdapter 的主要职责就是调用 Handler 的方法来处理请求，并且适配不同类型的处理器。每个实现类有不同的方法签名和交互方式。
  - RequestMappingHandlerAdapter 会解析 @RequestMapping @RequestParam、@RequestBody 等注解，将 HTTP 请求参数转换为方法参数，再调用目标方法 —— 这是适配器模式 “兼容不同接口” 的典型应用。
  - SimpleControllerHandlerAdapter 则适用于实现了 org.springframework.web.servlet.mvc.Controller 接口的老式控制器。
...
Spring Cloud 3.x 版本中不再推荐使用 Hystrix，并将其替换为 Resilience4j。使用注解熔断器 @EnableCircuitBreaker开启。Spring Cloud Circuit Breaker提供了抽象的统一熔断器接口，支持 Resilience4j、Hystrix、Sentinel 等具体熔断器实现。
...
- 2.Spring Cloud Netflix(已停止维护，已废弃,相关组件已被替换其他实现),开源组件集成，包括Eureka、Hystrix、Ribbon、Feign、Zuul等核心组件。
...
- 7.Spring Cloud OpenFeign
  - 基于CircuitBreaker熔断器和LoadBalancer负责均衡的(旧版本基于Ribbon和Hystrix的) **声明式服务调用组件**，可以动态创建基于Spring MVC注解的接口实现用于服务调用。(旧版本)
  - 
