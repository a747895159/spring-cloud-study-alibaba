...
async function postGoogleJson<T>(pathname: string, body: unknown): Promise<T> {
  ...
  if (!apiKey) throw new Error("需要 GOOGLE_API_KEY 或 GEMINI_API_KEY");
...
async function generateWithGemini(
  ...
  console.log("正在使用 Gemini 生成图像...", imageConfig);
...
async function generateWithImagen(
  ...
  if (imageSize === "4K") {
    console.error("警告：Imagen 模型不支持 4K 图像尺寸，改用 2K。");
  }
