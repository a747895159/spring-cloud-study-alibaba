...
**重要：在开始之前阅读整个文档。** 全文涵盖了关键的 XML 模式规则和格式要求。错误的实现可能导致 PowerPoint 无法打开的无效 PPTX 文件。

## 技术指南

### 模式合规性
- **`<p:txBody>` 中的元素顺序**：`<a:bodyPr>`、`<a:lstStyle>`、`<a:p>`
- **空白字符**：为具有前导/尾随空格的 `<a:t>` 元素添加 `xml:space='preserve'`
- **Unicode**：转义 ASCII 内容中的字符：`"` 变为 `&#8220;`
- **图像**：添加到 `ppt/media/`，在幻灯片 XML 中引用，设置尺寸以适应幻灯片边界
- **关系**：更新 `ppt/slides/_rels/slideN.xml.rels` 以获取每个幻灯片的资源
- **Dirty 属性**：将 `dirty="0"` 添加到 `<a:rPr>` 和 `<a:endParaRPr>` 元素以指示干净状态
...
### 常见模板复制陷阱：
- 复制后多张幻灯片引用同一备注幻灯片
- 来自不再存在的模板幻灯片的图像/媒体引用
- 未包含字体时的字体嵌入引用
- 布局 12-25 缺少 slideLayout 声明
