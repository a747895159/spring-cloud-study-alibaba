...
- 带背景/边框的 `<div>` - 变为形状
- `<img>` - 图像
- `class="placeholder"` - 为图表保留的空间（返回 `{ id, x, y, w, h }`）
...
- 在主体上使用 `display: flex` 以防止边距折叠破坏溢出验证
- 使用 `margin` 进行间距（大小中包含内边距）
...
- Flexbox 有效 - 位置从渲染布局计算
- 在 CSS 中使用带 `#` 前缀的十六进制颜色
- **文本对齐**：在需要时使用 CSS `text-align`（`center`、`right` 等）作为 PptxGenJS 的提示，以便在文本长度略有偏差时进行文本格式化
...
- **框阴影**：`<div>` 元素上的 CSS `box-shadow` 转换为 PowerPoint 阴影
  - 仅支持外部阴影（忽略内部阴影以防止损坏）
  - 示例：`<div style="box-shadow: 2px 2px 8px rgba(0, 0, 0, 0.3);">`
  - 注意：PowerPoint 不支持内部阴影，将被跳过

### 图标和渐变

- **关键：绝不使用 CSS 渐变（`linear-gradient`、`radial-gradient`）** - 它们不会转换为 PowerPoint
...
<div class="content">
  ...
  <p>带有 <b>粗体</b>、<i>斜体</i>、<u>下划线</u> 的文本。</p>
...
2. **内容不得溢出主体** - 报告溢出及确切测量值
3. **CSS 渐变** - 报告不支持的渐变使用
4. **文本元素样式** - 报告文本元素上的背景/边框/阴影（仅允许在 div 上）

**所有验证错误一起收集并报告**在单个错误消息中，允许您一次修复所有问题，而不是逐个修复。
...
在使用 `html2pptx` 将 HTML 转换为幻灯片后，您将使用 PptxGenJS 添加动态内容，如图表、图像和其他元素。

### ⚠️ 关键规则

#### 颜色
- **绝不使用 `#` 前缀**与 PptxGenJS 中的十六进制颜色 - 导致文件损坏
...
- **> 365 天**：使用每年分组（例如，"2023"、"2024"）
- **验证**：只有 1 个数据点的图表可能表示该时间段聚合不正确
...
slide.addChart(pptx.charts.BAR, [{
    ...
    valAxisMaxVal: 8000,
    valAxisMinVal: 0,  // 对计数/金额使用 0；对于聚类数据（例如，4500-7100），考虑从最小值附近开始
...
slide.addChart(pptx.charts.LINE, [{
    ...
    // valAxisMinVal: 30,  // 首选：对于聚类在范围内的数据（例如，32-55 或评级 3-5），从最小值附近开始轴以显示变化
    // 可选：图表颜色
...
**关键**：饼图需要**单个数据系列**，所有类别都在 `labels` 数组中，相应的值在 `values` 数组中。
...
**将图表颜色与您选择的设计调色板对齐**，确保数据可视化具有足够的对比度和独特性。调整颜色以用于：
- 相邻系列之间的强对比
- 幻灯片背景上的可读性
- 可访问性（避免仅红绿组合）

```javascript
// 示例：受海洋调色板启发的图表颜色（已调整对比度）
...
slide.addChart(pptx.charts.LINE, [
    ...
    ...placeholders[0],
    chartColors: ["16A085", "FF6B9D"]  // 每系列一种颜色
});
```
